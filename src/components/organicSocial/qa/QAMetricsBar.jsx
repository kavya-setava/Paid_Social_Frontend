...
import { METRIC_CONFIG, globalFont } from './QADashboardConstants';

const QAMetricsBar = ({ tickets, activeTab, setActiveTab }) => {

  const counts = useMemo(() => {
    const c = { All: tickets.length };
    ['QA In Progress', 'QA Passed', 'QA Failed'].forEach(s => {
      c[s] = tickets.filter(t => t.qaStatus === s).length;
    });
    // Rework tab counts tickets QA has sent back at least once.
    c['Rework'] = tickets.filter(t => (t.reworkCount || 0) > 0).length;
    return c;
  }, [tickets]);
  ...
  return (
    <>
      <style>{`
        @keyframes qaCardPulse {
          0%,100% { opacity:1; }
          50% { opacity:0.75; }
        }
      `}</style>

      {/* Single row of cards — same layout as the Agent dashboard */}
      <div style={{
        marginBottom: '20px',
        width: '100%',
...
// ── Single Metric Card (Agent-style) ────────────────────────────────────────
